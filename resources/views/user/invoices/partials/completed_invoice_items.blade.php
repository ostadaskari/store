    ...
    @forelse($user_invoices as $invoice)
        ...
        <div class="accordion-item">
            <h2 class="accordion-header">
                ...
                <button class="accordion-button collapsed bg-green-50 text-gray-800 font-semibold flex flex-row items-center" type="button" data-bs-toggle="collapse" data-bs-target="#flush-collapse-{{ $invoice->id }}" aria-expanded="false" aria-controls="flush-collapse-{{ $invoice->id }}">
                    ...
                    <div class="meta-item text-success"><strong>مبلغ فاکتور:</strong><span> {{ number_format($invoice->total_amount) }} تومان</span></div>
                </button>
            </h2>
            {{-- Dynamic ID for Accordion Content --}}
            <div id="flush-collapse-{{ $invoice->id }}" class="accordion-collapse collapse" data-bs-parent="#accordionFlushCompleted">
                    ...
                    <div class="w-100 my-2 bg-light">
                        <div class="order-meta-grid">
                            ...
                            <div class="meta-item"><strong>وضعیت فاکتور:</strong>
                                @php
                                    // Status is assumed 'completed' for this tab's data
                                    $statusKey = 'completed';
                            ...
                            <div class="meta-item d-flex align-items-center gap-2">
                                ...
                                <svg width="16" height="16" fill="currentColor" class="bi bi-copy copy-icon text-success" viewBox="0 0 16 16" role="button" style="cursor: pointer;">
                        ...
                        <div class="order-progress" style="position: relative; height: 40px; margin-top: 80px;">
                            ...
                            <div class="progress" style="height: 10px;">
                                @php
                                    // Since this is the completed tab, we set it to 100% and success color
                                    $progressWidth = '100%';
                                @endphp
                                <div class="progress-bar bg-success progress-bar-striped" role="progressbar" style="width: {{ $progressWidth }};">
                                </div>
                            </div>
                            <!-- home -->
                            <svg width="40" height="40" viewBox="0 0 16 16" class="bi bi-house-heart-fill house-icon">
                                <defs>
                                    <linearGradient id="fancyGradient-{{ $invoice->id }}" x1="0%" y1="0%" x2="100%" y2="100%">
                                        <stop offset="0%" stop-color="#38a169"/> {{-- Using green for completed --}}
                                        <stop offset="50%" stop-color="#48bb78"/>
                                        <stop offset="100%" stop-color="#68d391"/>
                                    </linearGradient>
                                </defs>
                                <path fill="url(#fancyGradient-{{ $invoice->id }})" d="M7.293 1.5a1 1 0 0 1 1.414 0L11 3.793V2.5a.5.5 0 0 1 .5-.5h1a.5.5 0 0 1 .5.5v3.293l2.354 2.353a.5.5 0 0 1-.708.707L8 2.207 1.354 8.853a.5.5 0 1 1-.708-.707L8 2.207 1.354 8.853a.5.5 0 1 1-.708-.707z"/>
                                <path fill="url(#fancyGradient-{{ $invoice->id }})" d="m14 9.293-6-6-6 6V13.5A1.5 1.5 0 0 0 3.5 15h9a1.5 1.5 0 0 0 1.5-1.5zm-6-.811c1.664-1.673 5.825 1.254 0 5.018-5.825-3.764-1.664-6.691 0-5.018"/>
                            </svg>
                        </div>
                    </div>
                </div>
            ...
            <div class="modal-dialog modal-xl modal-dialog-scrollable modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header justify-content-between">
                        ...
                        <h5 class="modal-title" id="invoiceModalLabel-{{ $invoice->id }}">فاکتور: #{{ $invoice->order_number }}</h5>
                    </div>
                            ...
                            <div class="row">
                                ...
                                <div class="col-12 col-lg-4 mb-2 colPrint">
                                    <div class="card shadow-sm h-100">
                                        <div class="card-header bg-success text-white text-right">خلاصه فاکتور</div>
                                        <div class="card-body text-right">
                                            ...
                                            <p><strong>وضعیت ارسال:</strong>
                                                <span class="{{ $statusClasses['completed'] }}">{{ $statusTexts['completed'] }}</span>
                                            </p>
